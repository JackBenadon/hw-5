...
df <- read.csv(path) # Insert your code here
df <- na.omit(df)
...
```{R}
lm_fit <- lm(df$median_house_value ~ df$latitude + df$longitude + df$housing_median_age + df$total_rooms + df$total_bedrooms + df$population + df$median_income + df$ocean_proximity ) 

summary(lm_fit)
```
Latitude: For every one degree increase in latitude, the median house value goes down by 2.624e+04

Longitude: For every one degree increase in latitude, the median house value goes down by  2.776e+04 

Housing median age: For every one degree increase in latitude, the median house value goes up by 1.082e+03

Total rooms: For every one degree increase in latitude, the median house value goes down by 6.445e+00

Total bedrooms: For every one degree increase in latitude, the median house value goes up by  1.378e+02

Population: For every one degree increase in latitude, the median house value goes down by 3.449e+01

Ocean proximity (inland): If it is inland, the median house value goes down by 3.926e+04

Ocean proximity (island): If it is island, the median house value goes up by 1.480e+05

Ocean proximity (near bay): If it is near bay, the median house value goes down by 4.102e+03

Ocean proximity (near ocean): If it is near the ocean, the median house value goes up by 4.058e+03
---
